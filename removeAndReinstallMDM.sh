...
if [[ `/usr/bin/dscl . read /Groups/admin GroupMembership | /usr/bin/grep -c $3` == 1 ]]; then
		echo "$3 is a local administrator, continuing to profile install."
    else
    echo "$3 is not an admin, promoting user."
    ...
    #########################################################
		...
		if [ ! -d /private/var/userToRemove ]; then
			...
			echo $3 >> /private/var/userToRemove/user
			else
				echo $3 >> /private/var/userToRemove/user
		fi
